#!/bin/bash
# A100 full-parameter GRPO, Qwen2.5 on GSM8k, NATIVE vLLM rollout (this is the path that
# actually learns: reward climbs, see docs/BENCHMARK.md). Runs directly on the node venv.
set -uo pipefail
...
EXP="${EXP:-a100_full_grpo_$(basename "$MODEL")}"
