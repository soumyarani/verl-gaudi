    ...
    def load_model(self) -> None:
            ...
            with ctx:
                ...
                try:
                    ...
                    for _vn, _vp in self.model.named_parameters():
                        _vm = dict(getattr(_vp, '__dict__', {}) or {})
                        _vm['_cls'] = type(_vp)
        ...
        if self._is_quant_with_inc():
            ...
            with HabanaMemoryProfiler() as m_inc:
                ...
                # .to() traversal order.)
                if not is_fake_hpu():
                    # HPU FIX: nn.Module.to('hpu') drops vLLM's per-param weight-loading metadata
                    # (weight_loader/output_dim/shard attrs on BasevLLMParameter), which breaks a later
                    # RL update_weights_from_ipc call. Capture before, restore after.
                    _VLLM_ATTRS = ('weight_loader', 'weight_loader_v2', 'output_dim', 'input_dim',
                                   'ignore_warning', 'is_gguf_weight', 'is_gguf_weight_type',
                                   'packed_dim', 'packed_factor', 'shard_id', 'is_metadata',
                                   'needs_scalar_to_array', 'data_container')
                    _saved_meta = {}
                    for _n, _p in self.model.named_parameters():
                        _saved_meta[_n] = {_a: getattr(_p, _a) for _a in _VLLM_ATTRS if hasattr(_p, _a)}
                    self.model = self.model.to("hpu")
                    for _n, _p in self.model.named_parameters():
                        for _a, _v in _saved_meta.get(_n, {}).items():
