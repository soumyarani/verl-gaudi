...
class ServerAdapter(BaseRollout):
    ...
    def __init__(
        ...
        # device_uuid is only consumed on the IPC weight-transfer path. On HPU
        # (is_support_ipc() is False) we use the CPU-mediated shm path, and HPU
        # modules are exclusive per process: this ServerAdapter shares its HPU
        # with the actual vLLM engine worker, so acquiring the device here just
        # to read a UUID fails with synStatus=8. Skip it when IPC is unsupported.
        self.device_uuid = get_device_uuid(get_device_id()) if is_support_ipc() else None
