...
class vLLMColocateWorkerExtension:
    ...
    def _update_weights(self, weights: list[tuple[str, torch.Tensor]], peft_config: dict, base_sync_done: bool):
        # HPU FIX: qwen2/vLLM AutoWeightsLoader shards merged params (qkv_proj, gate_up_proj) using a
        # `.shard_id` attribute on each weight tensor; verl's RL-update weights lack it -> tag by name.
        _SHARD_MAP = {'.q_proj.': 'q', '.k_proj.': 'k', '.v_proj.': 'v', '.gate_proj.': 0, '.up_proj.': 1}
        for _n, _t in weights:
            for _k, _sid in _SHARD_MAP.items():
                if _k in _n:
                    try:
                        _t.shard_id = _sid
                    except Exception:
