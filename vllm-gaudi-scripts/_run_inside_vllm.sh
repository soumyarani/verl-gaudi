...
sleep 2
echo "vllm env; in-process ray.init; RAYTMP=$RAYTMP"
# No `ray start` CLI (broken Sentinel bug in this image); verl's ray.init starts the head
# with HPU resources + agent timeout (RAY_agent_register_timeout_ms env).
python3.10 -m verl.trainer.main_ppo \
  ...
  trainer.n_gpus_per_node=1 trainer.nnodes=1 trainer.device=hpu trainer.use_v1=False \
  trainer.save_freq=-1 trainer.test_freq=-1 \
  trainer.total_epochs=1 trainer.total_training_steps=3 \
  trainer.project_name=verl_gaudi trainer.experiment_name=qwen05b_gsm8k_grpo_hpu_vllm
echo "VERL_RC=$?"
