#!/bin/bash
# Gaudi LoRA smoke: EAGER mode (PT_HPU_LAZY_MODE=0 set by the sbatch) per the repo's
# lazy-vs-eager cheat sheet — FSDP storage ops (use_orig_params=True, required by LoRA)
# are unsupported in lazy. Proven smoke protocol (bs16/resp128/n4) + LoRA-32, 5 steps.
set -uo pipefail
...
clean_ray
echo "LORA EAGER SMOKE: PT_HPU_LAZY_MODE=${PT_HPU_LAZY_MODE:-unset}"
...
run_verl() {
  python3.10 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=gaudi-lora32-eager-smoke \
