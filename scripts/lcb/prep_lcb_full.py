#!/usr/bin/env python3
"""FULL LiveCodeBench -> verl parquet (merges ALL releases test.jsonl..test6.jsonl, dedup by question_id).
Matches SDPO's dataset (date split train<2025-02-01). Run after downloading the 6 release jsonls.
Deviation from SDPO: test cases capped for HPU reward speed (set MAX_TESTS=None to match SDPO exactly)."""
import json, base64, zlib, pickle, os, random
from datetime import datetime
import pandas as pd
from huggingface_hub import hf_hub_download

CUTOFF = datetime(2025, 2, 1)
TIME_LIMIT = 6
MAX_TESTS = 15                 # SDPO uses all; cap for HPU reward speed
REV = "refs/pr/6"
RELEASES = ["test.jsonl", "test2.jsonl", "test3.jsonl", "test4.jsonl", "test5.jsonl", "test6.jsonl"]
random.seed(0)
CODE_PROMPT = ("You are a coding expert. You will be given a coding problem, and you need to write a correct "
    "Python program that matches the specification and passes all tests. The time limit is 1 second. You may "
    "start by outlining your thought process. In the end, please provide the complete code in a code block "
    "enclosed with ``` ```.\n\n{problem}")

def parse_sig(starter):
    return "def " + starter.split("def ")[1].split("Input\n")[0].strip()

def decode_tests(encoded):
    obj = pickle.loads(zlib.decompress(base64.b64decode(encoded)))
    return json.loads(obj) if isinstance(obj, str) else obj

def build(ex):
    if not ex.get("private_test_cases"): return None
    problem = ex["question_content"]
    if ex.get("starter_code", "").strip():
        problem += f"\n\nYour solution should have the following signature: ```python\n{parse_sig(ex['starter_code'])}\n```"
    fn_name = json.loads(ex["metadata"]).get("func_name", "") if ex.get("metadata", "").strip() else ""
    tests = decode_tests(ex["private_test_cases"])
    if MAX_TESTS and len(tests) > MAX_TESTS:
        tests = tests[:1] + random.sample(tests[1:], MAX_TESTS - 1)
    if not tests: return None
    gt = {"inputs": [t["input"] for t in tests], "outputs": [t["output"] for t in tests],
          "testtype": tests[0]["testtype"], "fn_name": fn_name, "time_limit": TIME_LIMIT}
    return {"data_source": "livecodebench",
            "prompt": [{"role": "user", "content": CODE_PROMPT.format(problem=problem)}],
            "ability": "code",
            "reward_model": {"style": "rule", "ground_truth": json.dumps(gt, ensure_ascii=False)},
            "extra_info": {"question_id": ex.get("question_id", ""), "difficulty": ex.get("difficulty", ""),
                           "contest_date": ex.get("contest_date", ""), "testtype": gt["testtype"], "n_tests": len(tests)}}

seen, all_ex = set(), []
for rel in RELEASES:
    p = hf_hub_download("livecodebench/code_generation_lite", rel, repo_type="dataset", revision=REV)
    n = 0
    for line in open(p):
        ex = json.loads(line)
        qid = ex.get("question_id", "")
        if qid in seen: continue
        seen.add(qid); all_ex.append(ex); n += 1
    print(f"{rel}: +{n} new (total {len(all_ex)})")

train, test = [], []
for ex in all_ex:
    b = build(ex)
    if not b: continue
    d = datetime.fromisoformat(ex["contest_date"])
    b["extra_info"]["split"] = "test" if d >= CUTOFF else "train"
    (test if d >= CUTOFF else train).append(b)
os.makedirs("/workspace/data/lcb_full", exist_ok=True)
pd.DataFrame(train).to_parquet("/workspace/data/lcb_full/train.parquet")
pd.DataFrame(test).to_parquet("/workspace/data/lcb_full/test.parquet")
print(f"FULL: train(<2025-02)={len(train)} test(>=2025-02)={len(test)} -> /workspace/data/lcb_full/")
print("train testtype:", pd.Series([r['extra_info']['testtype'] for r in train]).value_counts().to_dict())
