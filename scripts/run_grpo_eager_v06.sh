#!/bin/bash
# EAGER-MODE GRPO on GSM8k (diagnostic): PT_HPU_LAZY_MODE=0 to kill the lazy-view
# segfault (#2) + synStatus-26-on-resume (#3). 3 steps, flexible reward, wandb.
export VERL_PLATFORM=hpu PT_HPU_LAZY_MODE=0 VLLM_SKIP_WARMUP=true HF_HOME=/workspace/hf
...
python -m verl.trainer.main_ppo \
  ...
  trainer.default_local_dir=/workspace/ckpt/grpo_eager_v06 \
  trainer.save_freq=1 trainer.test_freq=-1 trainer.total_epochs=1 trainer.total_training_steps=3 \
  trainer.project_name=verl_air_gaudi trainer.experiment_name=grpo_eager_v06
echo "VERL_RC=$?"
