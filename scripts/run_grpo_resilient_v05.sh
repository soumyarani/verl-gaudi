#!/bin/bash
# Resilient full-ish GRPO on GSM8k, disaggregated on Gaudi (AIR, 4 HPU).
# - flexible reward (gsm8k.py patched: model answers in \boxed{} -> now scored)
# - response 256; wandb offline; checkpoint every 2 steps; resume_mode=auto
# - retry wrapper: intermittent HPU lazy-view segfault -> resume from last ckpt
export VERL_PLATFORM=hpu PT_HPU_LAZY_MODE=1 VLLM_SKIP_WARMUP=true HF_HOME=/workspace/hf
...
CKPT=/workspace/ckpt/grpo_gsm8k_resilient_v05
...
run_once () {
  ...
  python -m verl.trainer.main_ppo \
    ...
    trainer.save_freq=1 trainer.test_freq=-1 trainer.total_epochs=1 trainer.total_training_steps=20 \
    trainer.project_name=verl_air_gaudi trainer.experiment_name=grpo_gsm8k_resilient_v05
}

cleanup () {
  ray stop --force >/dev/null 2>&1
  P=$(hl-smi 2>/dev/null | sed -n '/Compute Processes/,/^+====/p' | awk '{print $3}' | grep -E '^[0-9]+$')
...
for attempt in $(seq 1 15); do
  ...
  if [ $RC -eq 0 ]; then echo "ALL_DONE_RC0"; break; fi
  echo "crash (RC=$RC); cleaning up + resuming from $CKPT ..."
  cleanup
done
echo "RESILIENT_FINISHED"
