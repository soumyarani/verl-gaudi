#!/bin/bash
# FULL GRPO on GSM8k, EAGER, DISK-SAFE (save_freq=-1 -> no checkpoint accumulation, the eviction cause).
...
python -m verl.trainer.main_ppo \
  ...
  actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu=2 actor_rollout_ref.actor.use_kl_loss=False \
  ...
  actor_rollout_ref.rollout.n=4 actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=2 \
  ...
  trainer.save_freq=-1 trainer.test_freq=-1 trainer.total_epochs=1 trainer.total_training_steps=40 \
  trainer.project_name=verl_air_gaudi trainer.experiment_name=grpo_eager_final
echo "VERL_RC=$?"
